...
export async function VehicleCard({
  ...
  const categoryName = vehicle.vehicle_categories
    ? locale === "fr"
      ? vehicle.vehicle_categories.name_fr
      : vehicle.vehicle_categories.name_en
    : null;
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden rounded-xl border border-border bg-background shadow-sm transition-shadow hover:shadow-md">
      <div className="relative aspect-[4/3] w-full bg-surface">
        ...
        {categoryName && (
          <span className="absolute left-3 top-3 rounded-full bg-primary-tint px-3 py-1 text-xs font-semibold text-primary-dark shadow-sm">
            {categoryName}
          </span>
        )}
      </div>
      <div className="flex flex-1 flex-col gap-3 p-4">
        <div className="flex items-start justify-between gap-2">
          <h3 className="line-clamp-1 text-lg font-semibold text-ink">{vehicle.name}</h3>
          <span className="shrink-0 rounded-full bg-action-tint px-2.5 py-1 text-[11px] font-semibold text-action-dark">
            {t("availableNow")}
          </span>
        </div>
        <ul className="flex min-h-[2.5rem] flex-wrap gap-x-3 gap-y-1 text-xs text-muted">
          {specs.map((spec, i) => (
        ...
        <p className="text-2xl font-extrabold text-action-dark">
        ...
        <div className="mt-auto flex flex-wrap gap-2 pt-2">
          <Link
            ...
            className="flex-1 rounded-full border border-border px-3 py-2 text-center text-sm font-semibold text-ink transition-colors hover:border-primary hover:text-primary-dark"
          >
            {t("view")}
          </Link>
          <Link
    ...
    <div className="flex h-full flex-col overflow-hidden rounded-xl border border-border bg-background shadow-sm">
      ...
      <div className="flex flex-1 flex-col gap-3 p-4">
        ...
        <div className="mt-auto flex gap-2 pt-2">
          <div className="h-9 flex-1 animate-pulse rounded-full bg-surface" />
          <div className="h-9 flex-1 animate-pulse rounded-full bg-surface" />
