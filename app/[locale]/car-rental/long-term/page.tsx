...
export async function generateMetadata(props: {
  ...
  const t = await getTranslations({ locale, namespace: "carRental.longTerm" });
  return buildPageMetadata({
    locale,
    path: "/car-rental/long-term",
    title: t("title"),
    description: t("metaDescription"),
  });
}

export default async function LongTermCarRentalPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("carRental.longTerm");
  const tShared = await getTranslations("carRental");
  const tCommon = await getTranslations("common");
  const vehicles = await getVehicles({ longTermOnly: true });
  ...
  return (
    <section className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <UseCaseHighlights
        highlights={[
          { icon: CreditCard, title: t("highlight1Title"), text: t("highlight1Text") },
          { icon: CalendarClock, title: t("highlight2Title"), text: t("highlight2Text") },
