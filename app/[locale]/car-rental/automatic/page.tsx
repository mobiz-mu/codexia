...
export async function generateMetadata(props: {
  ...
  const t = await getTranslations({ locale, namespace: "carRental.automatic" });
  return buildPageMetadata({
    locale,
    path: "/car-rental/automatic",
    title: t("title"),
    description: t("metaDescription"),
  });
}

export default async function AutomaticCarRentalPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("carRental.automatic");
  const tShared = await getTranslations("carRental");
  const tCommon = await getTranslations("common");
  const vehicles = await getVehicles({ transmission: "automatic" });
  ...
  return (
    <section className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <UseCaseHighlights
        highlights={[
          { icon: CheckCircle2, title: t("highlight1Title"), text: t("highlight1Text") },
          { icon: Car, title: t("highlight2Title"), text: t("highlight2Text") },
          { icon: ShieldCheck, title: t("highlight3Title"), text: t("highlight3Text") },
        ]}
      />
