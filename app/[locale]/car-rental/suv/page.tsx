...
import { Luggage, Mountain, ShieldCheck } from "lucide-react";
...
export async function generateMetadata(props: {
  ...
  const t = await getTranslations({ locale, namespace: "carRental.suv" });
...
export default async function SuvRentalPage({
  ...
  const t = await getTranslations("carRental.suv");
  const tShared = await getTranslations("carRental");
  const tCommon = await getTranslations("common");
  const vehicles = await getVehicles({ categorySlug: "suv" });
  ...
  return (
    <section className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <UseCaseHighlights
        highlights={[
          { icon: Luggage, title: t("highlight1Title"), text: t("highlight1Text") },
          { icon: Mountain, title: t("highlight2Title"), text: t("highlight2Text") },
