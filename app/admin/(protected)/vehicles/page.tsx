  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/admin/vehicles/new" className="rounded-full bg-primary px-4 py-2 text-sm font-semibold text-white">
          Add Vehicle
        </Link>
      </div>
            ...
            {vehicles.map((v) => (
              <tr key={v.id} className="border-b border-border last:border-0">
                <td className="px-4 py-2">
                  <Link href={`/admin/vehicles/${v.id}`} className="font-medium text-primary-dark">
                    {v.name}
                  </Link>
                </td>
