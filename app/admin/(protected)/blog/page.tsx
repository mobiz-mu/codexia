  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/admin/blog/new" className="rounded-full bg-primary px-4 py-2 text-sm font-semibold text-white">
          Add Post
        </Link>
      </div>
            ...
            {posts.map((p) => (
              <tr key={p.id} className="border-b border-border last:border-0">
                <td className="px-4 py-2">
                  <Link href={`/admin/blog/${p.id}`} className="font-medium text-primary-dark">
                    {p.title_en}
                  </Link>
                </td>
